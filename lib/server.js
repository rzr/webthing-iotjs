 ...
 */
function WebThingServer(things, port, sslOptions, additionalRoutes) {
  /**
   ...
   */
  {
    ...
    this.isMultipleThing = (this.things.constructor.name === 'MultipleThings');
  ...
  this.start = () => {
    try {
      ...
      console.error(`error: Fail to listen (check HTTP port "${this.port}")`);
      throw (err);
    }
  };
